...
def classify_email(email_text, max_length=128):
    ...
    probabilities = F.softmax(logits, dim=1)  # Apply softmax to obtain probabilities
    predicted_class_id = logits.argmax(dim=1).item()
    confidence_score = probabilities[0, predicted_class_id].item()  # Get confidence score
...
db = client["email_train_db3"]
collection = db["emails_train3"]
...
for doc in collection.find():
    ...
    collection.update_one(
        {"_id": doc["_id"]},
        {"$set": {"predicted_label": predicted_label, "confidence_score": confidence_score}}
    )
    
    print(f"Updated document {doc['_id']} with predicted label: {predicted_label}, confidence score: {confidence_score:.2f}")
