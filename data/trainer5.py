...
def process_emails(emails: List[Dict[str, Any]]) -> Dict[str, List]:
    ...
    logger.info("Categorization summary: update=%d, request=%d", update_count, request_count)
...
def main() -> None:
    ...
    best_checkpoint = run_hyperparameter_search(trainer, n_trials=5)
    if best_checkpoint:
        logger.info("Resuming training from best checkpoint: %s", best_checkpoint)
